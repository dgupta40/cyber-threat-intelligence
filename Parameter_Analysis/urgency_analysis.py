...
def prepare_factor_data(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if "n_articles" in df.columns:
        factors["n_articles_raw"] = df["n_articles"].fillna(
            0
        )  # Keep raw count for analysis
    ...
    factors["text_length"] = (
        df["clean_text"].str.len().fillna(0) / 1000
...
def analyze_factor_distributions(factor_data: pd.DataFrame):
    ...
    analysis_factors = [
        f for f in factor_data.columns if f not in ["days_old", "n_articles_raw"]
    ]
    ...
    factor_melted = factor_data[analysis_factors].melt(
...
def perform_weight_sensitivity_analysis(factor_data: pd.DataFrame, base_weights: dict):
    ...
    for factor in base_weights:
        ...
        for test_weight in test_weights:
            ...
            original_remaining_sum = sum(
                w for f, w in base_weights.items() if f != factor
            )
    ...
    most_sensitive = max(
        sensitivity_results.items(), key=lambda x: x[1]["level_change_percent"].max()
    )[0]
...
def test_weight_combinations(factor_data: pd.DataFrame):
    ...
    def generate_valid_combinations():
        ...
        for params in tqdm(
            param_grid, desc="Testing weight combinations", total=len(list(param_grid))
        ):
    ...
    def evaluate_combination(weights):
        ...
        balance = (
            1
            - (abs(low_pct - 1 / 3) + abs(med_pct - 1 / 3) + abs(high_pct - 1 / 3)) / 2
        )
...
def main():
    ...
    sensitivity_results = perform_weight_sensitivity_analysis(
        factor_data, CURRENT_WEIGHTS
    )
    ...
    with open(OUT_DIR / "urgency_analysis_summary.txt", "w") as f:
        ...
        for factor, contribution in current_analysis["avg_contributions"].items():
            f.write(
                f"- {factor}: {contribution:.4f} ({contribution/current_analysis['score_stats']['mean']*100:.1f}% of average score)\n"
            )
        f.write("\n")

        f.write("## Most Sensitive Factors\n")
        f.write(
            f"- Most sensitive factor: {sensitivity_results['most_sensitive_factor']}\n"
        )
        f.write("\n")
        ...
        for factor, weight in best_weights.items():
            f.write(
                f"- {factor}: {weight:.2f} (current: {CURRENT_WEIGHTS[factor]:.2f}, change: {weight-CURRENT_WEIGHTS[factor]:+.2f})\n"
            )
        f.write("\n")
        ...
        f.write(
            f"- Score Variance: {optimization_results['best_combined']['score_variance']:.4f}\n"
        )
        f.write(
            f"- Balance Score: {optimization_results['best_combined']['balance']:.4f}\n"
        )
        f.write("\n")
        ...
        f.write(
            f"- Medium: {optimization_results['best_combined']['med_pct']*100:.1f}%\n"
        )
        f.write(
            f"- High: {optimization_results['best_combined']['high_pct']*100:.1f}%\n"
        )
        f.write("\n")
        ...
        for factor, weight in best_weights.items():
            f.write(
                f"    '{factor}': {weight:.2f},  # {'' if factor in CURRENT_WEIGHTS else 'NEW '}"
            )
