...
def analyze_tfidf_params(df):
    ...
    for min_df in min_df_values:
        for max_df in max_df_values:
            ...
            filtered_low_freq = sum(1 for term, count in word_freq.items() if count < min_df)
            ...
            results.append({
                ...
                'vocab_percent_of_reference': vocab_size / reference_vocab_size * 100 if reference_vocab_size > 0 else 0
            })
    ...
    plot_heatmap(results_df, min_df_values, max_df_values, 'vocab_size', 
                 'Vocabulary Size by TF-IDF Parameters', 
                 'vocabulary_size_heatmap.png')
    
    # 2. Sparsity heatmap
    plot_heatmap(results_df, min_df_values, max_df_values, 'sparsity_pct', 
                 'Matrix Sparsity (%) by TF-IDF Parameters', 
                 'sparsity_heatmap.png')
    
    # 3. Vocabulary coverage
    plot_heatmap(results_df, min_df_values, max_df_values, 'vocab_coverage_pct', 
                 'Vocabulary Coverage (%) by TF-IDF Parameters', 
                 'coverage_heatmap.png')
    
    # 4. Plot percent of reference vocabulary
    plot_heatmap(results_df, min_df_values, max_df_values, 'vocab_percent_of_reference', 
    ...
    fixed_min_df = 3
    fixed_min_df_results = results_df[results_df.min_df == fixed_min_df].sort_values('max_df')
    
    fig, axes = plt.subplots(1, 2, figsize=(12, 5))
    
    # Vocabulary size by max_df (with min_df=3)
    axes[0].plot(fixed_min_df_results['max_df'], fixed_min_df_results['vocab_size'], 'bo-')
    ...
    axes[1].plot(fixed_min_df_results['max_df'], fixed_min_df_results['sparsity_pct'], 'ro-')
    ...
    fixed_max_df = 0.8
    fixed_max_df_results = results_df[results_df.max_df == fixed_max_df].sort_values('min_df')
    
    fig, axes = plt.subplots(1, 2, figsize=(12, 5))
    
    # Vocabulary size by min_df (with max_df=0.8)
    axes[0].plot(fixed_max_df_results['min_df'], fixed_max_df_results['vocab_size'], 'bo-')
    ...
    axes[1].plot(fixed_max_df_results['min_df'], fixed_max_df_results['sparsity_pct'], 'ro-')
    ...
    with open(OUTPUT_DIR / "tfidf_recommendations.txt", "w") as f:
        ...
        f.write(f"- Vocabulary coverage: {optimal_row['vocab_coverage_pct']:.1f}% of unique terms\n")
        f.write(f"- Matrix sparsity: {optimal_row['sparsity_pct']:.1f}%\n")
        f.write(f"- Removed {optimal_row['removed_low_freq']} low-frequency terms\n")
        f.write(f"- Removed {optimal_row['removed_high_freq']} high-frequency terms\n\n")
...
def find_optimal_params(results_df):
    ...
    results_df['score'] = (
        ...
        (100 - results_df['sparsity_pct']) * 0.3 +  # Less sparsity is better (to a point)
