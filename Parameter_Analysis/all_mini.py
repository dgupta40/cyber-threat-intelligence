...
def generate_test_sets(df):
    ...
    test_sets['full_eval'] = df.sample(min(FULL_EVAL_SAMPLE, len(df))).clean_text.tolist()
    
    # Larger sample for speed testing
    test_sets['speed'] = df.sample(min(SPEED_TEST_SAMPLE, len(df))).clean_text.tolist()
    
    # Domain-specific examples
    test_sets['cyber_examples'] = [pair[0] for pair in CYBER_TEST_PAIRS] + [pair[1] for pair in CYBER_TEST_PAIRS]
...
def benchmark_models(test_sets):
    ...
    for model_name in MODELS:
        ...
        try:
            ...
            # Get model info
            model_results['embedding_dimension'] = model.get_sentence_embedding_dimension()
            ...
            for bs in batch_sizes:
                ...
                start_time = time.time()
                _ = model.encode(test_sets['speed'][:min(bs*10, len(test_sets['speed']))], 
                                batch_size=bs, show_progress_bar=False)
                encoding_time = time.time() - start_time
                encoding_times.append({
                    'batch_size': bs,
                    'time': encoding_time,
                    'texts_per_second': min(bs*10, len(test_sets['speed'])) / encoding_time
                })
            ...
            eval_embeddings = model.encode(test_sets['full_eval'], show_progress_bar=True)
    ...
    try:
        ...
        for i in range(n_clusters):
            ...
            if len(cluster_points) > 1:
                ...
                within_sim += sim_matrix.sum() / (len(cluster_points) * (len(cluster_points) - 1))
...
def evaluate_semantic_understanding(model, test_pairs):
    ...
    high_expected_avg = sum(r['actual_similarity'] for r in high_expected_pairs) / len(high_expected_pairs)
    low_expected_pairs = [r for r in results if r['expected_similarity'] <= 0.3]
    low_expected_avg = sum(r['actual_similarity'] for r in low_expected_pairs) / len(low_expected_pairs)
    ...
    return {
        ...
        'contrast_ratio': high_expected_avg / low_expected_avg if low_expected_avg > 0 else float('inf')
    }
...
def visualize_embeddings(embeddings, model_name):
    ...
    # Perform t-SNE
    tsne = TSNE(n_components=2, random_state=42, perplexity=min(30, max_tsne_samples-1))
...
def create_comparative_visualizations(results):
    ...
    for i, name in enumerate(model_names):
        plt.annotate(name, (model_sizes[i], semantic_scores[i]), 
                   fontsize=9, ha='center', va='bottom', rotation=20)
    ...
    for model_name, data in results.items():
        if 'error' not in data and 'cluster_metrics' in data and 'error' not in data['cluster_metrics']:
            model_names.append(model_name)
            silhouette_scores.append(data['cluster_metrics']['silhouette_score'])
            within_cluster_similarities.append(data['cluster_metrics']['avg_within_cluster_similarity'])
    
    # Create subplots
    fig, (ax1, ax2) = plt.subplots(1, 2, figsize=(15, 6))
    
    # Silhouette scores
    ax1.bar(model_names, silhouette_scores, color=['red' if name == 'all-MiniLM-L6-v2' else 'blue' for name in model_names])
    ...
    ax2.bar(model_names, within_cluster_similarities, color=['red' if name == 'all-MiniLM-L6-v2' else 'blue' for name in model_names])
...
def generate_report(results):
    ...
    for model_name, data in results.items():
        ...
        if 'encoding_performance' in data:
            ...
            largest_batch = max(data['encoding_performance'], key=lambda x: x['batch_size'])
        ...
        if 'semantic_understanding' in data:
            ...
            delta_score = 5 * (1 - min(1, data['semantic_understanding']['avg_delta'] * 2))
    ...
    if len(model_scores) > 0:
        # Find max speed
        speeds = [scores.get('_texts_per_second', 0) for scores in model_scores.values()]
    ...
    with open(OUTPUT_DIR / "embedding_model_report.md", "w") as f:
        ...
        f.write("## Overview\n\n")
        f.write("This report analyzes different sentence transformer models to determine the optimal embeddings for cybersecurity text data.\n\n")
        
        f.write("## Model Comparison\n\n")
        f.write("| Model | Embedding Quality | Speed | Efficiency | Cyber Understanding | Total Score |\n")
        f.write("| --- | --- | --- | --- | --- | --- |\n")
        ...
        for model_name, scores in model_scores.items():
            if 'error' in scores:
                ...
                f.write(f"| {model_name} | {scores['embedding_quality']:.2f} | {scores['speed']:.2f} | {scores['efficiency']:.2f} | {scores['cyber_understanding']:.2f} | {scores['total_score']:.2f} |\n")
        
        f.write("\n## Recommendation\n\n")
        if 'error' not in best_model[1]:
            f.write(f"Based on our analysis, **{best_model[0]}** is the recommended model with a score of {best_model[1]['total_score']:.2f}/10.\n\n")
            
            # Compare with your current model
            current_model_score = model_scores.get('all-MiniLM-L6-v2', {}).get('total_score', 0)
            if best_model[0] == 'all-MiniLM-L6-v2':
                f.write("Your current model choice (all-MiniLM-L6-v2) is optimal based on our evaluation metrics.\n\n")
            elif current_model_score > 0:
                score_diff = best_model[1]['total_score'] - current_model_score
                if score_diff > 1:
                    f.write(f"Switching from your current model (all-MiniLM-L6-v2) to {best_model[0]} could provide significant improvements (score difference: +{score_diff:.2f}).\n\n")
                else:
                    f.write(f"Your current model (all-MiniLM-L6-v2) performs well, with only a small difference ({score_diff:.2f}) compared to the top model.\n\n")
        else:
            f.write("Could not determine a recommendation due to errors in the evaluation.\n\n")
        ...
        if 'error' not in current_data:
            f.write(f"- **Embedding Dimension**: {current_data.get('embedding_dimension', 'N/A')}\n")
            f.write(f"- **Model Size**: {current_data.get('model_size_mb', 'N/A'):.2f} MB\n")
            
            if 'encoding_performance' in current_data:
                best_speed = max(current_data['encoding_performance'], key=lambda x: x['texts_per_second'])
                f.write(f"- **Encoding Speed**: {best_speed['texts_per_second']:.2f} texts/second (batch size {best_speed['batch_size']})\n")
            
            if 'cluster_metrics' in current_data and 'error' not in current_data['cluster_metrics']:
                f.write(f"- **Clustering Quality (Silhouette)**: {current_data['cluster_metrics']['silhouette_score']:.4f}\n")
            
            if 'semantic_understanding' in current_data:
                f.write(f"- **Cybersecurity Semantic Understanding**: Contrast ratio of {current_data['semantic_understanding']['contrast_ratio']:.2f}\n")
            
            f.write("\nThis model offers an excellent balance of performance, size, and semantic understanding for cybersecurity text. ")
            f.write("The 384-dimensional embeddings provide sufficient representational power while keeping computational requirements manageable. ")
            ...
            if weaknesses:
                f.write(f"Areas for potential improvement include {', '.join(weaknesses)}. ")
        else:
...
def main():
    ...
    best_model = max(model_scores.items(), key=lambda x: x[1].get('total_score', 0))
    if 'error' not in best_model[1]:
        logging.info(f"Recommended model: {best_model[0]} (Score: {best_model[1]['total_score']:.2f}/10)")
    else:
