...
from sklearn.metrics import classification_report, multilabel_confusion_matrix, f1_score, precision_recall_fscore_support
import joblib
...
try:
    ...
    GPU_AVAILABLE = False
    logging.warning("RAPIDS cuML not found - using CPU only. Install with: pip install cudf-cu11 cuml-cu11 --extra-index-url=https://pypi.nvidia.com")
...
def prepare_data(log):
    ...
    label_counts = {cat: sum(1 for labels in y_labels if cat in labels) for cat in CATEGORIES}
...
def analyze_label_distribution(data, log):
    ...
    y_binary = data['mlb'].transform(data['df'].apply(
        lambda row: list(set(rule_labels(row['clean_text']))) or ["Other"], 
        axis=1
    ))
...
def compare_feature_sets(data, log):
    ...
    plt.bar(x - width/2, performance_df['Training Time (s)'], width, label='Training Time')
    plt.bar(x + width/2, performance_df['Prediction Time (s)'], width, label='Prediction Time')
...
def tune_random_forest(data, log):
    ...
    log.info(f"Testing {len(param_grid['n_estimators']) * len(param_grid['max_depth'])} parameter combinations")
                ...
                try:
                    ...
                    if GPU_AVAILABLE:
                        if max_depth is None:
                            ...
                            model = cuRF(n_estimators=n_estimators, max_depth=100, random_state=42)
                        else:
    ...
    for max_depth in param_grid['max_depth']:
        ...
        if len(subset) > 0:
            plt.plot(subset['n_estimators'], subset['f1_macro'], 
                    marker='o', label=f'max_depth={depth_label}')
...
def analyze_feature_importance(data, best_model, log):
    ...
    all_feature_names = np.concatenate([tfidf_features, numeric_features, sbert_features])
    ...
    for i, model in enumerate(best_model):
        if hasattr(model, 'feature_importances_'):
            ...
            if GPU_AVAILABLE:
                ...
                importances = model.feature_importances_.to_numpy() if hasattr(model.feature_importances_, 'to_numpy') else model.feature_importances_
            else:
    ...
    for i, fi in enumerate(feature_importances):
        ...
        tfidf_importance = sum(importances[np.array([name.startswith('sbert_') == False and name not in numeric_features for name in fi['feature_names']])])
        numeric_importance = sum(importances[np.array([name in numeric_features for name in fi['feature_names']])])
        sbert_importance = sum(importances[np.array([name.startswith('sbert_') for name in fi['feature_names']])])
...
def analyze_multilabel_confusion(y_true, y_pred, categories, log):
    ...
    for i, category in enumerate(categories):
        ...
        # Add metrics as text
        plt.figtext(0.5, 0.01, f"Precision: {precision:.2f}, Recall: {recall:.2f}, F1: {f1:.2f}",
...
def main():
    ...
    if tuning_results and 'best_model' in tuning_results:
        importance_analysis = analyze_feature_importance(data, tuning_results['best_model'], log)
    else:
    ...
    if tuning_results and 'best_model' in tuning_results and tuning_results['best_model'] is not None:
        ...
        joblib.dump({
            ...
            "feature_importance": importance_analysis['feature_type_importance'] if importance_analysis else None,
    ...
    with open(summary_file, 'w') as f:
        ...
        if tuning_results and 'y_pred' in tuning_results:
            report = classification_report(data['y_test'], tuning_results['y_pred'], 
                                        target_names=CATEGORIES, 
                                        output_dict=True,
                                        zero_division=0)
