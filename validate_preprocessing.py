...
def run_validation(input_file='data/processed/master.csv', output_dir='validation_results'):
    ...
    for col in critical_columns:
        ...
        if null_count > 0:
            results['issues'].append(f"Missing data: {null_count} null values in '{col}'")
    ...
    if empty_text > 0:
        results['issues'].append(f"Empty text: {empty_text} records with empty 'clean_text'")
    ...
    if len(invalid_sources) > 0:
        results['issues'].append(f"Invalid sources: {len(invalid_sources)} records with unexpected source values")
    ...
    if html_rate > 0.01:  # More than 1% with HTML
        results['issues'].append(f"HTML remnants: {html_count} samples ({html_rate:.1%}) contain HTML tags")
    ...
    unnormalized = sample['clean_text'].astype(str).str.contains(cve_pattern, case=False).sum()
    unnorm_rate = unnormalized / sample_size
    if unnorm_rate > 0.05:  # More than 5% with unnormalized CVEs
        results['warnings'].append(f"CVE normalization: {unnormalized} samples ({unnorm_rate:.1%}) contain unnormalized CVE references")
        print(f" {unnormalized} samples ({unnorm_rate:.1%}) contain unnormalized CVE references")
    else:
    ...
    if sentiment_min < -1.0 or sentiment_max > 1.0:
        results['issues'].append(f"Sentiment range: found values outside [-1, 1] range: [{sentiment_min}, {sentiment_max}]")
        print(f" Sentiment values outside expected range: [{sentiment_min}, {sentiment_max}]")
    else:
    ...
    if 'cvss_score' in nvd_df.columns:
        invalid_cvss = nvd_df[(nvd_df['cvss_score'].notna()) & 
                             ((nvd_df['cvss_score'] < 0) | (nvd_df['cvss_score'] > 10))]
        if len(invalid_cvss) > 0:
            results['issues'].append(f"CVSS scores: {len(invalid_cvss)} records have values outside [0, 10] range")
    ...
    if 'primary_cve' in nvd_df.columns and 'mentioned_cves' in nvd_df.columns:
        ...
        for _, row in sample_nvd.iterrows():
            if pd.notna(row['primary_cve']):
                ...
                if isinstance(mentioned, str):
                    try:
                        mentioned = eval(mentioned)  # Convert string representation of list to actual list
                    except:
                        mentioned = []
        ...
        if mismatch_rate > 0.05:  # More than 5% mismatches
            results['issues'].append(f"CVE extraction: {cve_mismatch} NVD records ({mismatch_rate:.1%}) missing primary CVE in mentioned_cves")
            print(f" {cve_mismatch} NVD records ({mismatch_rate:.1%}) have CVE extraction issues")
        else:
    ...
    thn_sample = df[df['source'] == 'thehackernews'].sample(min(5, len(df[df['source'] == 'thehackernews'])))
