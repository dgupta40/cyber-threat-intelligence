...
out_path = PROC_DIR / f"linked_thn_nvd_{pd.Timestamp.now():%Y%m%d_%H%M}.parquet"
df_linked.to_parquet(out_path, index=False)
print(f"Linked dataset saved to: {out_path}")