...
def main():
    ...
    df_thn["mentioned_cves"]  = df_thn["cves"].apply(lambda x: list(set(x)) if isinstance(x, list) else [])
    df_thn["cves_from_text"]   = df_thn["clean_text"].apply(lambda t: list(set(CVE_RE.findall(t))))
    df_thn["all_cves"]         = df_thn.apply(lambda r: list(set(r.mentioned_cves + r.cves_from_text)), axis=1)
    ...
    emb_nvd = sbert.encode(df_nvd.clean_text.tolist(), batch_size=32, show_progress_bar=True)
    np.save(MODEL_DIR / "sbert_nvd.npy", emb_nvd)
    emb_thn = sbert.encode(df_thn.clean_text.tolist(), batch_size=32, show_progress_bar=True)
    ...
    df_nvd["earliest_article_date"] = df_nvd.cve_id.map(
        lambda c: df_thn.loc[cve_map[c], "published_date"].min() if c in cve_map else pd.NaT
    )
