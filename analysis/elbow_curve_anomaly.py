...
def analyze_contamination_parameter(df: pd.DataFrame):
    ...
    contamination_values = np.linspace(
        0.01, 0.1, 20
    )  # Test from 1% to 10% with finer granularity
    results = []

    log.info(
        f"Testing {len(contamination_values)} contamination values between {min(contamination_values):.2f} and {max(contamination_values):.2f}"
    )
    ...
    log.info(
        f"Suggested optimal contamination (derivative method): {optimal_contamination:.4f}"
    )
    log.info(
        f"Alternative contamination (leveling off): {alternative_contamination:.4f}"
    )
...
def main():
    ...
    log.info(
        f"Analysis complete. Optimal contamination value: {optimal_contamination:.4f}"
    )
    log.info(
        f"Visualization saved to {OUTPUT_DIR / 'contamination_elbow_analysis.png'}"
    )
    log.info(
        f"Detailed results saved to {OUTPUT_DIR / 'contamination_analysis_results.csv'}"
    )

    log.info("To use this value, update your detect_emerging() function with:")
    log.info(
        f"    iso = IsolationForest(contamination={optimal_contamination:.4f}, random_state=42)"
    )
