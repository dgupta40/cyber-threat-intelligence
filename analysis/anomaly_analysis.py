...
def generate_summary_stats(df):
    ...
    print(f"Overlaps -> Z only: {only_z}, S only: {only_s}, IF only: {only_if}, Z+S: {zs}, Z+IF: {zi}, S+IF: {si}, All: {zsi}")
    print(f"{'='*50}\n")

    # By month
    if 'published_dt' in df:
        df['published_dt'] = pd.to_datetime(df['published_dt'], errors='ignore')
        monthly = df.groupby(df['published_dt'].dt.to_period('M'))['emerging'].agg(['count','sum'])
...
def plot_time_series(df, path):
    ...
    weekly = df.set_index('published_dt').resample('W')['emerging'].agg(['sum','count'])
...
def visualize_high_dimensional_data(df, path):
    ...
    scatter = ax.scatter(Y[:,0], Y[:,1], c=sample['emerging'].astype(int), cmap='coolwarm', alpha=0.6)
...
def analyze_category_distribution(df, path):
    ...
    for c in cats:
        ...
        records.append({'Category': name, 'Type': 'Emerging', 'Count': df[c][df['emerging']].sum()})
        records.append({'Category': name, 'Type': 'Non-Emerging', 'Count': df[c][~df['emerging']].sum()})
